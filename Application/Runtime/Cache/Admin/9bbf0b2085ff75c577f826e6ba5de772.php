				...
				<div class="table-toolbar">
					...
					<div class="pull-right">
						<form id="App-search">
							...
							<a href="<?php echo U('Admin/Wxhongbao/wxtxorder/');?>" class="btn btn-success" data-loader="App-loader" data-loadername="提现订单" data-search="App-search">
				...
				<table id="App-table" class="table table-bordered table-hover">
					...
					<tbody>
						<?php if(is_array($cache)): $i = 0; $__LIST__ = $cache;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr id="item<?php echo ($vo["id"]); ?>">
								...
								<td class=" "><?php echo ($vo["billno"]); ?></td>
								...
								<td class=" "><?php echo (date('Y/m/d H:i',$vo["txtime"])); ?></td>
								<td class=" ">
								    <?php if($vo["status"] == 0): ?><span class="label label-warning">未发放</span>
                                    <?php elseif($vo["status"] == 1): ?><span class="label label-info">待领取</span>
                                    <?php else: ?><span class="label label-default">已领取</span><?php endif; ?>
								</td>
								<td class="center ">
...
<script type="text/javascript">
	...
	$('#exportTxorder').on('click',function(){
		...
		window.open("<?php echo U('Admin/Wxhongbao/wxtxorderExport');?>/id/"+chk);
	})
</script>
<!--/全选特效封装-->